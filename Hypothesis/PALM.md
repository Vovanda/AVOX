...
В современных LLM-системах проблема обновления и персонализации знаний остаётся нерешённой. **RAG** (Retrieval-Augmented Generation) позволяет временно подмешивать новые данные не изменяя саму модель. **Fine-tuning** всей модели LLM решает проблему, но требует значительных вычислительных ресурсов и времени.
...
- **Интеграция с RAG**: при запросе новые данные сначала подаются на вход LLM,работая как классический RAG, а затем, при накоплении **критической массы**, используются при дообучении персонализированных словев.
- "Актуализация данных внутри LLM при достижении "критической массы" данных**: обучение происходит не постоянно, а когда накапливается достаточное количество данных.
...
При тюнинге адаптеров важно формировать тренировочную выборку так, чтобы модель **усваивала новые данные, но не теряла критически важные знания** ниже представлен подход ранжирования данных.
...
- **Гибридное селективное внимание (Hybrid Selective Attention)**:
  ...
  - Менее критичная или временная информация остаётся в RAG до её окончательной валидации и интеграции в модель.
...
Подобная стратегия позволяет эффективно интегрировать новые данные в модель, не нарушая уже накопленные знания, и обеспечивает стабильность и актуальность информации.
...
1. **Разрыв в согласованности информации**:
   - В RAG новые данные не доступны все сразу, но и вывод из-за недостатка контекста может противоречить актуальной информации. При этом при различных, не сильно четких, запросах итог системы может быть также различным.
   - В предложенном подходе свежие данные сначала испотльзуются в RAG и потом попадают в обучающую выборку. RAG в PALM нацелен на небольшой объем данных - это устраняет *разрыв в соглассовонности* и увеличивает стабильность работы системы RAG-системы.

2. **Устаревание информации**:
   - В RAG-системах достаточно сложно удалить или исправить устаревшие данные, что связано с необходимостью затраты значительных ресурсов для отслеживания и актуализации информации при большом объеме данных. Задача не является тривиальной с точки зрения алгоритмов и ресурсы часто тратятся без значительного улучшения точности ответов.
   - В стратегии PALM система ориентирована на сохранение наиболее важных данных, понимая их в контексте целого. Данные, которые система усвоила, но которые со временем становятся менее востребованными, могут быть постепенно забыты. Для объективности стоит отметить, что мы также тратим ресурсы на постоянный тюнинг адаптера, что влечёт за собой определённые издержки, однако это позволяет сохранять актуальность и целосность знаний, повышая качество ответов.
...
1. **Continual Learning (CL)**:
   - **CL** представляет собой семейство методов и подходов, которые нацелены на решение проблемы **катастрофического забывания**, где модель должна продолжать учиться на новых данных, не забывая старые знания. В рамках **CL** существует несколько подходов, таких как:
     - **Regularization-based methods** (например, Elastic Weight Consolidation — EWC), которые накладывают ограничения на изменение важнейших параметров модели, чтобы не забывать предыдущие знания.
     - **Replay-based methods** (например, Experience Replay или Generative Replay), где старые данные или их эмбеддинги сохраняются и периодически используются для обучения, чтобы предотвратить забывание.
     - **Parameter Isolation** и **Dynamic Architectures**, где модель обновляется, добавляя новые слои или компоненты для обучения на новых данных без потери уже усвоенных знаний.
     
   В контексте **PALM**, он может быть рассмотрен как **прикладной метод CL**, где используются адаптеры для динамической интеграции новых знаний в модель, поддерживая баланс между сохранением старых знаний и адаптацией к новому контексту. 

2. **Sliding Window Sampling**:
   - Этот подход учитывает только последние данные, игнорируя старые знания. Это подходит для ситуаций, когда актуальность новых данных превосходит старые, но не позволяет сохранять долгосрочные связи и контекст.

3. **Bayesian Importance Estimation**:
   - Динамическое обновление весов важности документов, которое позволяет выделять более значимые данные для обучения. Это решение более сложное в реализации и требует применения байесовских методов для оценки важности каждого документа.


## Заключение

**PALM (Personalized Adaptive Learning Model)** сочетает адаптеры и RAG для эффективной персонализации LLM-систем. Этот подход решает проблемы классического RAG, такие как разрыв в согласованности информации, устаревание данных и ограниченность контекстного окна. В отличие от традиционных методов, PALM интегрирует актуальные знания через адаптеры, оставляя менее важные данные в RAG.

PALM можно рассматривать как метод в рамках **continual learning**, где модель адаптируется к новым данным, сохраняя старые знания. Это позволяет динамически обновлять модель, минимизируя ресурсы и поддерживая стабильность знаний.

Особое внимание стоит уделить безопасности. **PALM** поддерживает изоляцию персональных данных каждого пользователя, гарантируя, что персонализированные данные не пересекаются между пользователями. Это позволяет безопасно интегрировать индивидуальные предпочтения и контекст, обеспечивая высокую степень конфиденциальности и защиту персональных данных, что критично для многопользовательских систем и приложений с высоким уровнем требований к безопасности.